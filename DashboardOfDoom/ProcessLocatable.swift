...
protocol ProcessLocatable {
    var location: Location { get }
}

func sortByDistance<T: ProcessLocatable>(_ items: [T], from location: Location, limit: Int = 0) -> [T] {
...
func maxDistance<T: ProcessLocatable>(_ items: [T], from location: Location, limit: Int = 0) -> Measurement<UnitLength> {
...
func minDistance<T: ProcessLocatable>(_ items: [T], from location: Location, limit: Int = 0) -> Measurement<UnitLength> {
    let sorted = sortByDistance(items, from: location, limit: limit)
...
func isItemInPolygon<T: ProcessLocatable>(_ item: T, polygon: [Location]) -> Bool {
...
func filterItemsInPolygon<T: ProcessLocatable>(_ items: [T], polygon: [Location]) -> [T] {
