...
public final class WeatherController: ProcessControllerProtocol {
    public init() {}
    ...
    public func refreshData(for location: Location) async throws -> [ProcessSensor] {
        ...
        if let placemark = await LocationManager.reverseGeocodeLocation(latitude: location.latitude, longitude: location.longitude) {
            let sensor = ProcessSensor(name: "", location: location, placemark: placemark, customData: ProcessMetadata(["icon": current.symbolName]), measurements: measurements, timestamp: Date.now)
            data.append(sensor)
        }
        return data
    }
