...
public final class RadiationController: ProcessControllerProtocol {
        ...
        for nearestStation in nearestStations {
            ...
            if let placemark = await LocationManager.reverseGeocodeLocation(location: nearestStation.location) {
                let sensor = ProcessSensor(
                    name: nearestStation.name, location: nearestStation.location, placemark: placemark, customData: ProcessMetadata(["icon": "atom"]),
